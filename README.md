...
An autonomous, solar-powered edge computing cluster built entirely from decommissioned Android smartphones. It distributes multi-agent spatial simulations (flocking, herding, attraction logic) across constrained mobile hardware and streams live telemetry to a client UI — no cloud infrastructure involved.

Full rationale, architecture tiers, and technology decisions live in [Blueprint.md](Blueprint.md).

## How it works

A Rust simulation core compiles to WebAssembly (`wasm32-wasip1`) and runs under the [WasmEdge](https://wasmedge.org/) runtime on each donor phone via Termux. The core computes agent physics and streams state over a WebSocket as raw binary (via `bytemuck`/`#[repr(C)]` struct casting) rather than JSON, so a JS/WebGL client can read coordinates straight out of an `ArrayBuffer` with `DataView` at high frame rates without garbage-collector overhead.
...
See Blueprint.md §3 for details on each tier and §4 for the full stack decision matrix (Wasm vs. Docker, Rust vs. Go, K3s vs. K8s, WebSockets+FlatBuffers vs. REST+JSON, React Native/Expo for the client).
...
This project doubles as a portfolio piece demonstrating compute-per-watt engineering for constrained, battery/solar-powered devices — see Blueprint.md §2 for the full context.
